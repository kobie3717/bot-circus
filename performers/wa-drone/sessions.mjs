...
const DB_PATH = '/root/claude-telegram-bot-friday/data/sessions.db';
...
export function getOrCreateSession(chatId) {
  ...
  if (existing) {
    // Update last_used
    db.prepare('UPDATE sessions SET last_used = ? WHERE chat_id = ?').run(now, chatId);
    return { sessionId: existing.session_id, isNew: false };
  }

  // Create new session
  const sessionId = randomUUID();
  db.prepare('INSERT INTO sessions (chat_id, session_id, created_at, last_used) VALUES (?, ?, ?, ?)')
