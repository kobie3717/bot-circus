 ...
 * @param {string[]} tags - Tags for categorization (default: ['telegram', 'friday'])
 * @param {string} category - Memory category (default: 'learning')
 * @returns {Promise<{ok: boolean, error?: string}>}
 */
export async function storeMemory(content, tags = ['telegram', 'friday'], category = 'learning') {
  try {
...
export async function autoStoreConversation(userMessage, assistantResponse) {
  ...
  for (const fact of facts) {
    await storeMemory(fact, ['telegram', 'friday', 'auto-captured']);
  }
}
