...
import { getOrCreateSession, clearSession, getSessionInfo, getStats, getLastSummary } from './sessions.mjs';
...
console.log('🦀 Starting Claw Telegram Bot...');
...
async function buildSystemPrompt(userMessage = '', chatId = null) {
  ...
  let fullPrompt = `You are Claw 🦀 — an AI super assistant. You are running via Claude Code CLI, connected to Telegram.
...
You can check servers, read logs, edit code, run commands — everything OpenClaw could do.
...
bot.command('status', async (ctx) => {
  ...
  try {
    const claudeProcess = spawn(CLAUDE_CLI_PATH, [
      ...
      '--system-prompt', 'You are Claw 🦀. Run a quick health check: docker ps --format "{{.Names}} {{.Status}}", pm2 jlist | jq -r ".[] | .name + \" \" + .pm2_env.status", curl -sf http://localhost:4000/health | jq .status, df -h / | tail -1. Report concisely with bullet points.',
  ...
  if (subcommand === 'recent') {
    ...
    try {
      ...
      if (lines.length === 0 || lines[0].includes('No memories')) {
        await ctx.reply('🦀 No recent Claw memories');
      } else {
        const formatted = lines.join('\n');
        await ctx.reply(`🧠 *Recent Claw Memories*\n\n\`\`\`\n${formatted}\n\`\`\``, { parse_mode: 'Markdown' });
      }
    } catch (e) {
...
bot.command('scavenge', async (ctx) => {
  ...
  try {
    ...
    const scavengePrompt = `Extract useful patterns, code snippets, and ideas from this GitHub repo that are relevant to our projects (FlashVault, WhatsAuction, Claw). Focus on: architecture patterns, interesting libraries, deployment strategies, monitoring/logging setups, API designs.
...
bot.command('jobs', async (ctx) => {
  ...
  try {
    ...
    console.error('[Claw] Global jobs error:', err.message);
  }
...
async function handleTextMessage(ctx) {
  ...
  try {
    ...
    try {
      ...
      if (shouldShare) {
        const written = await writeSharedKnowledge(content, category, confidence, domain, 'Claw');
...
const WEBHOOK_PORT = 7710;
const WEBHOOK_URL = 'https://whatshubb.co.za/webhook/claw';
const WEBHOOK_SECRET = process.env.WEBHOOK_SECRET || 'claw-webhook-x9k2m7p4';
...
async function startWebhook() {
  try {
    ...
    server.listen(WEBHOOK_PORT, '127.0.0.1', () => {
      ...
      console.log('🦀 Claw is online and listening (webhook mode)!');
        ...
        .then(token => {
          if (token) {
            ...
            registerTaskHandler('build', async (payload) => {
              ...
              return { received: true, brief, note: 'Logged for review — Claw processes via Claude' };
            });
