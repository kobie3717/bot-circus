...
export async function unfoldMemory(memoryId, level = 'summary', ringToken = null) {
  // Note: This implementation assumes ring token auth will be added later
  // For now, returns null since we don't have a way to get the ring token in this file
  // Real implementation would need to import/receive _ringToken from the bot system
  try {
    const res = await fetch(
      `${CIRCUS_URL}/api/v1/memory-commons/unfold?memory_id=${encodeURIComponent(memoryId)}&level=${level}`,
      {
        headers: ringToken ? { 'Authorization': `Bearer ${ringToken}` } : {},
