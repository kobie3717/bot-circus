 ...
 * Returns { signal: 'positive'|'negative'|'correction'|'neutral', score, confidence, evidence }
 */
export function detectSatisfaction(userMessage) {
  if (!userMessage) return { signal: 'neutral', score: 0.5, confidence: 0.3 };
  const msg = userMessage.trim().toLowerCase();

  // Positive signals
  if (/^(thanks|thank you|perfect|exactly|great|awesome|love it|that worked|yes!|brilliant|nice|sorted|lekker|dankie|cheers)/.test(msg)) {
    return { signal: 'positive', score: 0.85, confidence: 0.9, evidence: userMessage.slice(0, 100) };
  }
  if (/worked|fixed|solved|resolved|that's it|that's the one/.test(msg) && msg.length < 60) {
    return { signal: 'positive', score: 0.8, confidence: 0.8, evidence: userMessage.slice(0, 100) };
  }

  // Negative signals
  if (/^(no|wrong|not what|doesn't work|didn't work|failed|error|broken|that's wrong|nope|not right)/.test(msg)) {
    return { signal: 'negative', score: 0.2, confidence: 0.8, evidence: userMessage.slice(0, 100) };
  }

  // Correction signal — user re-explains after bot response
  if (msg.startsWith('no,') || msg.startsWith('actually') || msg.startsWith('wait,') || msg.startsWith('not quite')) {
    return { signal: 'correction', score: 0.3, confidence: 0.7, evidence: userMessage.slice(0, 100) };
  }

  return { signal: 'neutral', score: 0.5, confidence: 0.3 };
}
