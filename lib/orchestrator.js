...
export class Orchestrator {
  ...
  /**
   * Validate all performer workspaces against contract
   * @private
   */
    ...
    try {
      ...
      const processMessage = async (message) => {
        ...
        try {
          ...
          // Send response — guest mode uses answerGuestQuery, normal uses sendMessage
          if (message.replyFn) {
            await message.replyFn(response);
          } else if (client) {
      ...
      const onMessage = async (botId, chatId, userId, text, replyFn) => {
        // Guard: drop messages for quarantined performers
        if (this.quarantined.has(botId)) {
          this.logger.warn({ botId, chatId, userId }, 'dropping message — performer quarantined');
          return;
        }

        try {
          queue.enqueue({ chatId, userId, text, replyFn });
  ...
  async start(botId = null) {
    ...
    // Validate performer workspaces
    await this._validatePerformers();

    // Register SIGUSR2 handler for on-demand revalidation
    process.on('SIGUSR2', () => {
  ...
  /**
   * Reset rate limiter and message count for a bot (or global limiter if no botId)
   * @param {string|null} botId
   */
  resetBudget(botId = null) {
    if (botId) {
      const bot = this.bots.get(botId);
      if (!bot) {
        throw new Error(`Bot ${botId} not running`);
      }
      bot.queue.resetRateLimit?.();
      bot.messageCount = 0;
      this.logger.info({ botId }, 'Bot budget reset');
    } else {
      this.globalRateLimiter.reset();
      this.logger.info('Global rate limiter reset');
    }
  }
