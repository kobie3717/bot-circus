...
const DB_PATH = process.env.SESSIONS_DB_PATH || '/root/claude-telegram-bot/data/sessions.db';
...
export function getOrCreateSession(chatId) {
  ...
  if (existing) {
    // message_count == 0 means stub row (post-clear summary holder). Rotate to fresh UUID and treat as new.
    if (existing.message_count === 0) {
      const sessionId = randomUUID();
      db.prepare('UPDATE sessions SET session_id = ?, created_at = ?, last_used = ?, message_count = 1 WHERE chat_id = ?')
        .run(sessionId, now, now, chatId);
      return { sessionId, isNew: true };
    }
    // TTL check — Claude sessions expire after ~7 days of inactivity.
    // If last_used > TTL, rotate to a fresh session to avoid "No conversation found" errors.
    const SESSION_TTL_MS = parseInt(process.env.SESSION_TTL_DAYS || '6', 10) * 24 * 60 * 60 * 1000;
    const isExpired = (now - existing.last_used) > SESSION_TTL_MS;
    if (isExpired) {
      console.log(`[Sessions] Session for chat ${chatId} expired (TTL), rotating to new`);
      const sessionId = randomUUID();
      db.prepare('UPDATE sessions SET session_id = ?, created_at = ?, last_used = ?, message_count = 1 WHERE chat_id = ?')
        .run(sessionId, now, now, chatId);
      return { sessionId, isNew: true };
 ...
 * Rotates session_id to a fresh UUID, resets counters, and preserves summary
 * in the same row so getLastSummary() can inject it into the next session.
 * @param {number} chatId - Telegram chat ID
 * @param {string} summary - Optional session summary to persist before rotation
 */
export function clearSession(chatId, summary = null) {
  const existing = db.prepare('SELECT summary FROM sessions WHERE chat_id = ?').get(chatId);
  const preservedSummary = summary || existing?.summary || null;
  ...
  if (preservedSummary) {
    ...
    db.prepare(`
      INSERT INTO sessions (chat_id, session_id, created_at, last_used, message_count, summary)
      VALUES (?, ?, ?, ?, 0, ?)
    `).run(chatId, stubId, now, now, preservedSummary);
  }

  console.log(`Cleared session for chat ${chatId}${preservedSummary ? ' (summary preserved)' : ''}`);
}
...
export function cleanExpiredSessions(hours = 24) {
  ...
  // Rows with a summary: convert to stub (preserve context, reset session)
  const stubbed = db.prepare(`
    UPDATE sessions SET session_id = lower(hex(randomblob(16))), message_count = 0, last_used = ?
    WHERE last_used < ? AND summary IS NOT NULL AND message_count > 0
  `).run(now, cutoff).changes;

  // Rows without summary: delete
  const deleted = db.prepare('DELETE FROM sessions WHERE last_used < ? AND summary IS NULL').run(cutoff).changes;

  if (stubbed > 0 || deleted > 0) {
    console.log(`Cleaned sessions (older than ${hours}h): ${deleted} deleted, ${stubbed} stubbed with summary`);
  }
